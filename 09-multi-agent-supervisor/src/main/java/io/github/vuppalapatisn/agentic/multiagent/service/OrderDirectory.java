package io.github.vuppalapatisn.agentic.multiagent.service;
...
/** Trusted order facts, the policy clauses, and the {@code R1} fraud boundary. */
@Component
public class OrderDirectory {
    ...
    public List<String> policyClauses() {
        return List.of(
                ...
                "RP-ESCALATE-LEGAL: any mention of legal action, a chargeback or a regulator is escalated, never refunded automatically.");
    }

    public List<String> clauseIds() {
        return policyClauses().stream().map(clause -> clause.substring(0, clause.indexOf(':'))).toList();
    }
